...
![](./img/Gantt-excluded-days-within.png)
...
excluded dates. As shown here ![](./img/Gantt-long-weekend-look.png)
...
initial date for the milestone and the “duration” of the task this way:
*initial date*+*duration*/2.
...
More info in:
<https://github.com/d3/d3-time-format/tree/v4.0.0#locale_format>

### Axis ticks

The default output ticks are auto. You can custom your `tickInterval`,
like `1day` or `1week`.
...
Comments can be entered within a gantt chart, which will be ignored by
...
Styling of the Gantt diagram is done by defining a number of CSS
classes. During rendering, these classes are extracted from the file
located at src/diagrams/gantt/styles.js
...
You can style or hide the marker for the current date. To style it, add
a value for the `todayMarker` key.
...
This is done by defining the `ganttConfig` part of the configuration
object. How to use the CLI is described in the
[mermaidCLI](../config/mermaidCLI.md) page.

mermaid.ganttConfig can be set to a JSON string with config parameters
...
It is possible to bind a click event to a task. The click can lead to
either a javascript callback or to a link which will be opened in the
current browser tab. **Note**: This functionality is disabled when using
...
- callback is the name of a javascript function defined on the page
  displaying the graph, the function will be called with the taskId as
  the parameter if no other arguments are specified.

Beginner’s tip—a full example using interactive links in an html
context:
